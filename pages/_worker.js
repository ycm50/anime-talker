// ============================================================
// 酒馆 - Cloudflare Pages _worker.js
// 处理: 静态页面、REST API、WebSocket 聊天
// ============================================================

// ======================== 工具函数 ========================
const CHARS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
function genId() { let r = ''; for (let i = 0; i < 8; i++) r += CHARS[Math.random() * CHARS.length | 0]; return r; }
...
function genColor() { return COLORS[Math.random() * COLORS.length | 0]; }

// ======================== 全局状态 ========================
const AI = { baseUrl: '', apiKey: '', model: '', worldBook: '', jailbreakPrompt: '' };
const users = new Map();
const history = [];
const MAX_H = 200;

function broadcast(d) { const m = JSON.stringify(d); for (const [ws] of users) { try { ws.send(m); } catch (_) {} } }

// ======================== KV 持久化 ========================
async function loadKV(env) {
  if (!env.JIUGUAN_KV) return;
  try {
    const s = await env.JIUGUAN_KV.get('settings', 'json');
    if (s) Object.assign(AI, s);
    const msgs = await env.JIUGUAN_KV.get('messages', 'json');
    if (Array.isArray(msgs)) { history.length = 0; for (const m of msgs) history.push(m); }
  } catch (_) {}
}

async function saveHist(env) {
  if (!env.JIUGUAN_KV) return;
  try { await env.JIUGUAN_KV.put('messages', JSON.stringify(history.slice(-MAX_H))); } catch (_) {}
}

let saveTimer = null;
function schedSave(env) {
  if (saveTimer) clearTimeout(saveTimer);
  saveTimer = setTimeout(() => { saveHist(env); saveTimer = null; }, 2000);
}

// ======================== AI 流式调用 ========================
async function callAI(msgs, model, env, onChunk, onDone, onError, signal) {
  const isCF = model?.startsWith('@cf/');
  if (isCF) {
    if (!env.AI) { onError('Workers AI 未绑定'); return; }
    try {
      const r = await env.AI.run(model, { messages: msgs, stream: true }, { signal });
      let full = '';
      for await (const c of r) { if (c.response) { full += c.response; onChunk(c.response); } }
      onDone(full);
    } catch (e) { if (e.name !== 'AbortError') onError('AI 失败: ' + (e.message || e)); else onError('已停止生成'); }
    return;
  }
  const baseUrl = (AI.baseUrl || env.BASE_URL || 'https://api.openai.com').replace(/\/+$/, '');
  const apiKey = AI.apiKey || env.API_KEY || '';
  try {
    const resp = await fetch(baseUrl + '/v1/chat/completions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + apiKey },
      body: JSON.stringify({ model, messages: msgs, stream: true }), signal
    });
    if (!resp.ok) { const t = await resp.text().catch(() => ''); onError('API 失败 (' + resp.status + '): ' + t.substring(0, 200)); return; }
    const reader = resp.body.getReader();
    const dec = new TextDecoder(); let full = '', buf = '';
    while (true) {
      ...
      for (const line of lines) {
        ...
        try { const j = JSON.parse(d); const delta = j.choices?.[0]?.delta?.content; if (delta) { full += delta; onChunk(delta); } if (j.choices?.[0]?.finish_reason === 'stop') break; } catch (_) {}
      }
    }
    onDone(full);
  } catch (e) { if (e.name !== 'AbortError') onError('AI 失败: ' + e.message); else onError('已停止生成'); }
}
...
async function handleRequest(request, env) {
  ...
  // API: 服务器信息
  if (path === '/api/info') {
    return Response.json({ name: '酒馆', version: '2.0.0 (Pages)', online: users.size, cpp_available: false });
  }

  // API: 获取 AI 设置
  if (path === '/api/ai/settings' && request.method === 'GET') {
    return Response.json({ baseUrl: AI.baseUrl || '', apiKey: AI.apiKey || '', model: AI.model || '', worldBook: AI.worldBook || '', jailbreakPrompt: AI.jailbreakPrompt || '' });
  }
  ...
  if (path === '/api/ai/settings' && request.method === 'POST') {
    try {
      ...
      if (body.baseUrl !== undefined) AI.baseUrl = body.baseUrl;
      if (body.apiKey !== undefined) AI.apiKey = body.apiKey;
      if (body.model !== undefined) AI.model = body.model;
      if (body.worldBook !== undefined) AI.worldBook = body.worldBook;
      if (body.jailbreakPrompt !== undefined) AI.jailbreakPrompt = body.jailbreakPrompt;
      if (env.JIUGUAN_KV) await env.JIUGUAN_KV.put('settings', JSON.stringify(AI));
  ...
  if (path === '/api/ai/models' && request.method === 'POST') {
    try {
      const body = await request.json();
      const baseUrl = body.baseUrl || AI.baseUrl || env.BASE_URL;
      const apiKey = body.apiKey || AI.apiKey || env.API_KEY;
      let models = [];
      if (env.AI) {
        models.push({ id: '@cf/meta/llama-3.1-8b-instruct', owned_by: 'Meta (CF)' }, { id: '@cf/meta/llama-3-8b-instruct', owned_by: 'Meta (CF)' }, { id: '@cf/mistral/mistral-7b-instruct-v0.1', owned_by: 'Mistral (CF)' }, { id: '@cf/deepseek-ai/deepseek-r1-distill-qwen-32b', owned_by: 'DeepSeek (CF)' });
      }
      if (baseUrl && apiKey) {
        try {
          const resp = await fetch(baseUrl.replace(/\/+$/, '') + '/models', { headers: { 'Authorization': 'Bearer ' + apiKey } });
          if (resp.ok) { const d = await resp.json(); (d.data || d || []).filter(m => m.id).forEach(m => { if (!models.find(x => x.id === m.id)) models.push({ id: m.id, owned_by: m.owned_by || 'External' }); }); }
        } catch (_) {}
      }
      if (models.length === 0) models = [{ id: 'gpt-4o-mini', owned_by: 'OpenAI' }, { id: 'gpt-4o', owned_by: 'OpenAI' }];
      return Response.json({ success: true, models });
    } catch (e) { return Response.json({ success: false, error: e.message }); }
  }

  // API: 消息历史
  if (path === '/api/messages') return Response.json(history.slice(-Math.min(parseInt(url.searchParams.get('limit')) || 100, MAX_H)));
  if (path === '/api/messages/clear' && request.method === 'POST') { history.length = 0; if (env.JIUGUAN_KV) env.JIUGUAN_KV.put('messages', '[]').catch(() => {}); broadcast({ type: 'history_cleared' }); return Response.json({ success: true }); }

  // API: 导出
  if (path === '/api/export') {
    return new Response(JSON.stringify({ version: 1, exportedAt: new Date().toISOString(), messages: history }), {
      headers: { 'Content-Type': 'application/json', 'Content-Disposition': 'attachment; filename="jiuguan_chat_' + new Date().toISOString().slice(0,10) + '.json"' }
    });
  }
  ...
  if (path === '/api/import' && request.method === 'POST') {
    try {
      ...
      history.length = 0; for (const m of msgs) history.push(m);
      broadcast({ type: 'history_imported', messages: history.slice() });
      return Response.json({ success: true, count: history.length });
    } catch (e) { return Response.json({ error: '导入失败: ' + e.message }, { status: 400 }); }
  }

  // API: 在线用户
  if (path === '/api/users') {
    return Response.json(Array.from(users.values()).map(u => ({ id: u.id, name: u.name, color: u.color, joinTime: u.joinTime })));
  }
...
async function handleWebSocket(request, env) {
  ...
  const user = { id: genId(), name: '游客', color: genColor(), joinTime: new Date().toISOString(), pendingWb: false, pendingJb: false, abort: null };
  users.set(server, user);

  server.send(JSON.stringify({ type: 'welcome', userId: user.id, userColor: user.color, serverInfo: { cpp_available: false, ai_configured: !!(AI.baseUrl || AI.model || env.BASE_URL || env.AI) } }));
  server.send(JSON.stringify({ type: 'history', messages: history.slice(-100) }));
  const ul = () => Array.from(users.values()).map(u => ({ id: u.id, name: u.name, color: u.color, joinTime: u.joinTime }));
  broadcast({ type: 'userlist', users: ul() });
  ...
  server.addEventListener('message', async (event) => {
    try {
      ...
      switch (data.type) {
        case 'ai_chat': {
          const content = (data.content || '').trim(); if (!content) break;
          if (!AI.model) { server.send(JSON.stringify({ type: 'error', content: '请先选择 AI 模型' })); break; }
          const msgs = [{ role: 'system', content: '你是酒馆AI助手，在局域网聊天室中与用户对话。你友善、乐于助人，用中文回复。' }];
          if (user.pendingWb) { user.pendingWb = false; if (AI.worldBook?.trim()) msgs.push({ role: 'system', content: '【世界书】\n' + AI.worldBook.trim() }); }
          if (user.pendingJb) { user.pendingJb = false; if (AI.jailbreakPrompt?.trim()) msgs.push({ role: 'system', content: AI.jailbreakPrompt.trim() }); }
          if (Array.isArray(data.history)) for (const m of data.history) { if (m.role === 'user' || m.role === 'assistant') msgs.push(m); }
          msgs.push({ role: 'user', content });
          broadcast({ type: 'ai_chunk_start', sender: 'AI · ' + AI.model, senderColor: '#10a37f', timestamp: new Date().toISOString() });
          const um = { id: genId(), type: 'message', userId: user.id, sender: user.name, senderColor: user.color, content, timestamp: new Date().toISOString(), is_ai_response: false };
          history.push(um); if (history.length > MAX_H * 2) history.splice(0, history.length - MAX_H);
          broadcast(um); schedSave(env);
          const ctrl = new AbortController(); user.abort = ctrl;
          await callAI(msgs, AI.model, env,
            c => broadcast({ type: 'ai_chunk', chunk: c }),
            full => {
              broadcast({ type: 'ai_chunk_done', content: full, filtered: false, sender: 'AI · ' + AI.model, senderColor: '#10a37f', timestamp: new Date().toISOString() });
              history.push({ id: genId(), type: 'message', userId: 'ai', sender: 'AI · ' + AI.model, senderColor: '#10a37f', content: full, timestamp: new Date().toISOString(), is_ai_response: true, filtered: false });
              schedSave(env);
            },
            e => broadcast({ type: 'ai_chunk_done', error: true, content: e }),
            ctrl.signal
          );
          user.abort = null; break;
        }
        case 'set_name': {
          const old = user.name; user.name = (data.name || '').trim().substring(0, 20) || '匿名';
          users.set(server, user); broadcast({ type: 'system', content: old + ' 改名为 ' + user.name }); broadcast({ type: 'userlist', users: ul() }); break;
        }
        case 'edit_message': {
          const { messageId, newContent } = data; if (!messageId || !newContent) break;
          const idx = history.findIndex(m => m.id === messageId);
          if (idx === -1) { server.send(JSON.stringify({ type: 'error', content: '消息未找到' })); break; }
          if (history[idx].userId !== user.id) { server.send(JSON.stringify({ type: 'error', content: '只能编辑自己的消息' })); break; }
          history[idx].content = newContent.trim(); history[idx].edited = true; history.splice(idx + 1);
          broadcast({ type: 'message_edited', messageId, newContent: newContent.trim(), edited: true, truncateAfter: true, userId: user.id });
          break;
        }
        case 'stop_generation': {
          if (user.abort) { user.abort.abort(); user.abort = null; server.send(JSON.stringify({ type: 'generation_stopped', content: '✅ 已停止生成' })); }
          else server.send(JSON.stringify({ type: 'generation_stopped', content: '没有正在进行的生成' }));
          break;
        }
        case 'inject_worldbook': {
          if (!AI.worldBook?.trim()) { server.send(JSON.stringify({ type: 'error', content: '世界书内容为空，请先在 AI 设置中填写并保存世界书' })); break; }
          user.pendingWb = true; users.set(server, user);
          server.send(JSON.stringify({ type: 'inject_ack', content: '✅ 世界书已就绪，下一条消息生效' })); break;
        }
        case 'inject_jailbreak': {
          if (!AI.jailbreakPrompt?.trim()) { server.send(JSON.stringify({ type: 'error', content: '破限提示词内容为空，请先在 AI 设置中填写并保存破限提示词' })); break; }
          user.pendingJb = true; users.set(server, user);
          server.send(JSON.stringify({ type: 'inject_ack', content: '✅ 破限提示词已就绪，下一条消息生效' })); break;
        }
        case 'compress_context': {
          const hist = data.history || [];
          if (hist.length <= 6) { server.send(JSON.stringify({ type: 'compress_ack', content: '对话较短，无需压缩' })); break; }
          const keep = hist.slice(-6); keep.unshift({ role: 'system', content: '上一段对话共 ' + (hist.length - 6) + ' 条消息，已压缩为以下摘要：\n用户发起了关于角色扮演的对话，AI已经作出了相应回复。请继续当前角色和情节进行。' });
          server.send(JSON.stringify({ type: 'compress_ack', content: '✅ 已压缩: ' + hist.length + ' 条 → ' + keep.length + ' 条', history: keep })); break;
        }
        case 'ping': server.send(JSON.stringify({ type: 'pong' })); break;
        default: server.send(JSON.stringify({ type: 'error', content: '未知消息类型: ' + data.type }));
      }
    } catch (e) { server.send(JSON.stringify({ type: 'error', content: '内部错误: ' + e.message })); }
  });

  server.addEventListener('close', () => { users.delete(server); broadcast({ type: 'userlist', users: ul() }); });
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <div class="ai-section">
            ...
            <div class="ai-body" id="aiBody">
                ...
                <textarea id="worldBookInput" rows="3" placeholder="输入世界观设定、角色描述、背景故事等..."></textarea>
        ...
        <div class="sidebar-footer">🍺 酒馆 · Pages 版</div>
    </aside>
    <main class="main">
        <div class="chat-header">
            ...
            <div style="display:flex;align-items:center;gap:12px;">
                <span class="info" id="cppStatus">Pages</span>
...
(function(){
    ...
    function showError(t){ dom.overlay.classList.add('show'); dom.connectStatus.textContent = t; dom.connectError.style.display = 'block'; dom.retryBtn.style.display = 'block'; }
    ...
    function loadAISettingsFromServer(){
        fetch('/api/ai/settings').then(function(r){ return r.json(); }).then(function(data){
            ...
            if (data.apiKey) { state.aiApiKey = data.apiKey; dom.aiApiKey.value = data.apiKey; localStorage.setItem('jiuguan_ai_apikey', data.apiKey); }
            else { var k = localStorage.getItem('jiuguan_ai_apikey'); if(k){ state.aiApiKey = k; dom.aiApiKey.value = k; } }
